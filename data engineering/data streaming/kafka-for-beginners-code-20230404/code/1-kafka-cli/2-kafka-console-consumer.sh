...
# consuming(will consume/show messages that are sent after the command execution)
...
# consuming from beginning(all messages from start inside the topic will be displayed)
kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic second_topic --from-beginning

# display key, values and timestamp in consumer(ordered per partition)
